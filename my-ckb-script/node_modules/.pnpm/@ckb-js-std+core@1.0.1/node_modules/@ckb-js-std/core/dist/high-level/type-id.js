 ...
 */
import { SOURCE_OUTPUT, SOURCE_INPUT, SOURCE_GROUP_INPUT, SOURCE_GROUP_OUTPUT, CELL_FIELD_CAPACITY, loadScriptHash, loadCellByField, } from "@ckb-js-std/bindings";
import { hashTypeId } from "../hasher/hasherCkb";
import { loadCellTypeHash, loadInput, loadScript, QueryIter, } from "./high-level";
import { bytesEq } from "../bytes";
/**
 * Checks if a cell exists at the specified index and source
 * @internal
 */
function isCellPresent(index, source) {
    try {
...
function locateIndex() {
    ...
    const query = new QueryIter((i, s) => loadCellTypeHash(i, s), SOURCE_OUTPUT);
    const index = query
        .toArray()
 ...
 * ```
 */
export function validateTypeId(typeId) {
    if (isCellPresent(1, SOURCE_GROUP_INPUT) ||
 ...
 * @internal
 */
function loadIdFromArgs(offset) {
 ...
 * ```
 */
export function checkTypeId(offset) {
