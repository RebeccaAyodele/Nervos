// migrated from ccc
import { bytesConcat, bytesConcatTo } from "../bytes/index";
import { numFromBytes, numToBytes } from "../num/index";
export class Codec {
    ...
    map({ inMap, outMap, }) {
        return new Codec((encodable) => this.encode((inMap ? inMap(encodable) : encodable)), (buffer) => (outMap
            ? outMap(this.decode(buffer))
            : this.decode(buffer)), this.byteLength);
    }
    mapIn(map) {
 ...
 * @param littleEndian
 */
export function bigInt(byteLength, littleEndian = false) {
    return Codec.from({
        byteLength,
        encode: (numLike) => {
            return numToBytes(numLike, byteLength);
